...
const Home = () => {
    ...
    return (
        <>
            ...
            {/* Overview */}
            <section id="overview" className="border-b border-slate-200">
                <div className="max-w-6xl mx-auto px-4 sm:px-6 pt-8 pb-12 sm:pt-10 sm:pb-16">
                    <h2 className="text-2xl sm:text-3xl font-bold text-slate-900 mb-6">{pick('About', 'Über uns')}</h2>
                    ...
                    <div className="mt-10">
                        <p className="text-xs uppercase tracking-widest text-slate-500 mb-4">{pick('Affiliations', 'Affiliationen')}</p>
            ...
            {/* Members */}
            <section id="members" className="border-b border-slate-200">
                <div className="max-w-6xl mx-auto px-4 sm:px-6 py-12 sm:py-16">
                    <h2 className="text-2xl sm:text-3xl font-bold text-slate-900 mb-8">{pick('Our Group', 'Unsere Gruppe')}</h2>
                    ...
                    {researchAssistants.length > 0 && (
                        <>
                            <h2 className="text-2xl sm:text-3xl font-bold text-slate-900 mt-16 mb-8">{pick('Research Assistants', 'Wissenschaftliche Hilfskräfte')}</h2>
                            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-x-6 gap-y-10">
                    ...
                    {support.length > 0 && (
                        <>
                            <h2 className="text-2xl sm:text-3xl font-bold text-slate-900 mt-16 mb-8">{pick('Support Staff', 'Unterstützung')}</h2>
                            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-x-6 gap-y-10">
                                {support.map((m) => (
                                    <TeamMemberCard key={m.slug} member={m} showAffiliations={false} />
                                ))}
            ...
            <section id="contact">
                <div className="max-w-6xl mx-auto px-4 sm:px-6 py-12 sm:py-16">
                    <h2 className="text-2xl sm:text-3xl font-bold text-slate-900 mb-6">{pick('Contact', 'Kontakt')}</h2>
                    <ul className="space-y-3 text-slate-700 list-none p-0">
                        <li className="flex items-start gap-3">
                            <FiMail className="mt-1 text-slate-400" size={18} />
                            <span>
                                Christian Bartelt:{' '}
                                <a className="underline decoration-slate-300 underline-offset-4 hover:decoration-slate-900" href="mailto:bartelt@isse.tu-clausthal.de">
                                    bartelt@isse.tu-clausthal.de
                                </a>
                            </span>
                        </li>
                        <li className="flex items-start gap-3">
                            <FiMapPin className="mt-1 text-slate-400" size={18} />
                            <a
